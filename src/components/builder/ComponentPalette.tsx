...
const DraggablePaletteItem: React.FC<DraggablePaletteItemProps> = ({ item, gridLayout }) => {
  ...
  const IconComponent = {
    ...
    [BuilderComponentType.Section]: Box, // Placeholder, might need a better icon
    [BuilderComponentType.Container]: Box,
    [BuilderComponentType.Columns]: Columns,
    [BuilderComponentType.Grid]: Grid3X3, // Using Grid3X3 for CSS Grid
...
const ComponentPalette: React.FC<ComponentPaletteProps> = () => {
  ...
  const [activeTab, setActiveTab] = useState<string>("Layout"); // "All", "Layout", "Forms", "Media"
  ...
  const getCategoryItems = (category: string): PaletteItem[] => {
    ...
    if (category === "Layout") return filteredItems.filter(item => [BuilderComponentType.Section, BuilderComponentType.Container, BuilderComponentType.Columns, BuilderComponentType.Grid, BuilderComponentType.Spacer, BuilderComponentType.Divider].includes(item.type));
    if (category === "Basic") return filteredItems.filter(item => [BuilderComponentType.Text, BuilderComponentType.Heading, BuilderComponentType.Button, BuilderComponentType.Image, BuilderComponentType.Icon].includes(item.type));
  ...
  return (
    <div className="w-80 bg-theme-bg-primary border-r border-theme-border-primary flex flex-col">
      {/* Header with Search */}
