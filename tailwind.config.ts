import type { Config } from "tailwindcss";
import { fontFamily } from "tailwindcss/defaultTheme";

const config = {
  darkMode: ["class"],
  content: [
    './pages/**/*.{ts,tsx}',
    './components/**/*.{ts,tsx}',
    './app/**/*.{ts,tsx}',
    './src/**/*.{ts,tsx}',
  ],
  prefix: "",
  theme: {
    container: {
      center: true,
      padding: "2rem",
      screens: {
        "2xl": "1400px",
      },
    },
    extend: {
      colors: {
        border: "hsl(var(--border))",
        input: "hsl(var(--input))",
        ring: "hsl(var(--ring))",
        background: "hsl(var(--background))",
        foreground: "hsl(var(--foreground))",
        primary: {
          DEFAULT: "hsl(var(--primary))",
          foreground: "hsl(var(--primary-foreground))",
        },
        secondary: {
          DEFAULT: "hsl(var(--secondary))",
          foreground: "hsl(var(--secondary-foreground))",
        },
        destructive: {
          DEFAULT: "hsl(var(--destructive))",
          foreground: "hsl(var(--destructive-foreground))",
        },
        muted: {
          DEFAULT: "hsl(var(--muted))",
          foreground: "hsl(var(--muted-foreground))",
        },
        accent: {
          DEFAULT: "hsl(var(--accent))",
          foreground: "hsl(var(--accent-foreground))",
        },
        popover: {
          DEFAULT: "hsl(var(--popover))",
          foreground: "hsl(var(--popover-foreground))",
        },
        card: {
          DEFAULT: "hsl(var(--card))",
          foreground: "hsl(var(--card-foreground))",
        },
        wix: {
          dark: "#111422",
          light: "#eef1f2",
          accent: "#d3e22d",
          muted: "#909eb2",
          'accent-red': "#a4554a",
          'accent-gray': "#bbc8d3",
          'accent-teal': "#44adb5",
          'accent-lime': "#c9d098",
          'accent-pink': "#c29a95",
          'blue': '#3871FF', // Added for primary CTA blue
        },
        // New Dark Professional Theme
        'theme-bg-primary': '#0f0f0f',
        'theme-bg-secondary': '#1a1a1a',
        'theme-bg-tertiary': '#2a2a2a',
        'theme-bg-glass': 'rgba(255,255,255,0.05)',
        'theme-accent-primary': '#00d4ff',
        'theme-accent-secondary': '#ff6b6b',
        'theme-accent-success': '#51cf66',
        'theme-accent-warning': '#ffd43b',
        'theme-text-primary': '#ffffff',
        'theme-text-secondary': '#b3b3b3',
        'theme-text-muted': '#666666',
        'theme-text-accent': '#00d4ff',
        'theme-border-primary': 'rgba(255,255,255,0.1)',
        'theme-border-focus': 'rgba(0,212,255,0.5)',
        'theme-border-hover': 'rgba(255,255,255,0.2)',
      },
      borderRadius: {
        lg: "var(--radius)",
        md: "calc(var(--radius) - 2px)",
        sm: "calc(var(--radius) - 4px)",
      },
      fontFamily: {
        sans: ["MadeFor-Text", ...fontFamily.sans],
        heading: ["MadeFor-Display", ...fontFamily.sans],
      },
      fontSize: {
        'hero-title': ['6rem', { lineHeight: '1.1', fontWeight: '700' }],
        'hero-tagline': ['4.5rem', { lineHeight: '1.2', fontWeight: '400' }],
      },
      keyframes: {
        "accordion-down": {
          from: { height: "0" },
          to: { height: "var(--radix-accordion-content-height)" },
        },
        "accordion-up": {
          from: { height: "var(--radix-accordion-content-height)" },
          to: { height: "0" },
        },
        "fade-in": {
          from: { opacity: "0" },
          to: { opacity: "1" },
        },
        "fade-out": {
          from: { opacity: "1" },
          to: { opacity: "0" },
        },
        "slide-in-from-top": {
          from: { transform: "translateY(-100%)" },
          to: { transform: "translateY(0)" },
        },
        "slide-in-from-bottom": {
          from: { transform: "translateY(100%)" },
          to: { transform: "translateY(0)" },
        },
        "gradientShift": { // Added for animated gradient
          '0%, 100%': { 'background-position': '0% 50%' },
          '50%': { 'background-position': '100% 50%' },
        },
        "float": { // Added for particle animation
          '0%, 100%': { transform: 'translateY(0px) rotate(0deg)' },
          '50%': { transform: 'translateY(-20px) rotate(180deg)' },
        },
      },
      animation: {
        "accordion-down": "accordion-down 0.2s ease-out",
        "accordion-up": "accordion-up 0.2s ease-out",
        "fade-in": "fade-in 0.3s ease-in-out",
        "fade-out": "fade-out 0.3s ease-in-out",
        "slide-in-from-top": "slide-in-from-top 0.3s ease-in-out",
        "slide-in-from-bottom": "slide-in-from-bottom 0.3s ease-in-out",
        "gradientShift": "gradientShift 4s ease infinite", // Added for animated gradient
        "float": "float 6s ease-in-out infinite", // Added for particle animation
      },
    },
  },
  plugins: [require("tailwindcss-animate")],
} satisfies Config;

export default config;
